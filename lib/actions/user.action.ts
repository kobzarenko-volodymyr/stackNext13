...
export async function updateUser(params: UpdateUserParams) {
  try {
    ...
    // find and update User based on clerkId
...
export async function getAllUsers(params: GetAllUsersParams) {
  try {
    ...
    switch (filter) {
      case "new_users":
        // joinedAt, reputation - поле в Моделе
...
export async function getSavedQuestions(params: GetSavedQuestionsParams) {
  try {
    ...
    // searchQuery из mongoose
    // Filter query to select the Documents that Match the query
    // <typeof Question> тип по которому ищём
    const query: FilterQuery<typeof Question> = searchQuery
      ? // Проверили searchQuery ? title совпадает с преданным searchQuery
        { title: { $regex: new RegExp(searchQuery, "i") } }
      : {};

    let sortOptions = {};

    // Бери значения из файла constants
    switch (filter) {
      case "most_recent":
    ...
    // из Модели User берём Коллекцию Сохраненных
...
export async function getUserInfo(params: GetUserByIdParams) {
  try {
    ...
    // Берём userId из пропсов  await getUserInfo({ userId: params.id })
    const { userId } = params;
    ...
    // .countDocuments метод подсчета документов где у которых поле author равно user._id
...
export async function getUserQuestions(params: GetUserStatsParams) {
  try {
    ...
    // число Вопросов где User === Author
